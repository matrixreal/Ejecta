...
- (id)initWithProduct:(SKProduct *)productp {
    ...
    if( self = [super initWithContext:NULL object:NULL argc:0 argv:NULL] ) {        
...
- (void)finishPurchaseWithTransaction:(SKPaymentTransaction *)transaction {
	...
	[app invokeCallback:callback thisObject:jsObject
		argc:2 argv:(JSValueRef[]){jsError, jsTransaction}];
	
	
	JSValueUnprotect(app.jsGlobalContext, callback);
	JSValueUnprotect(app.jsGlobalContext, jsObject);
	callback = NULL;
}
